		...
		for _, registeredServer := range registeredServers {
			...
			if registeredServer.stamp.Proto == stamps.StampProtoTypeDNSCryptRelay ||
				...
				for i, currentRegisteredRelay := range proxy.registeredRelays {
					if currentRegisteredRelay.name == registeredServer.name {
						...
						if currentRegisteredRelay.stamp.String() != registeredServer.stamp.String() {
							...
							dlog.Infof("Total count of registered relays %v", len(proxy.registeredRelays))
						}
					}
				}
				if !found {
					dlog.Infof("Adding [%s] to the set of available relays", registeredServer.name)
				...
				if !found {
					...
					dlog.Infof("Total count of registered servers %v", len(proxy.registeredServers))
				}
			}
		}
	}
...
func (proxy *Proxy) exchangeWithUDPServer(
	...
	for tries := 2; tries > 0; tries-- {
		...
		dlog.Infof("[%v] Retry on timeout", serverInfo.Name)
	}
...
func (proxy *Proxy) clientsCountInc() bool {
	for {
		...
		if atomic.CompareAndSwapUint32(&proxy.clientsCount, count, count+1) {
			dlog.Infof("clients count: %d", count+1)
			return true
		}
	}
}
...
func (proxy *Proxy) processIncomingQuery(
	...
	dlog.Infof("Processing incoming query from %s", clientAddrStr)
